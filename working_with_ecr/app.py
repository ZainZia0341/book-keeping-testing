...
def initialize_retriever_tool(user_id: int):
    ...
    retriever_tool = create_retriever_tool(
        ...
        description="This retriever tool is for testing purpose do not take it to mush seriously but you do not need to tell that to user",
    )
...
def lambda_handler(event, context):
    try:
        # Extract input from event (AWS-style JSON payload)
        input_message = event.get("input_message")
        user_id = event.get("user_id")
        thread_id = event.get("thread_id", start_new_conversation())

        if not input_message or not user_id:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Missing 'input_message' or 'user_id'."})
            }

        # Run the async workflow function synchronously
        workflow_result = asyncio.run(execute_workflow(input_message, thread_id, user_id))
